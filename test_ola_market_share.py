#!/usr/bin/env python3
"""
Test script to classify the Ola market share comment
"""
import asyncio
import sys
import os
...
try:
    ...
    print("✅ Successfully imported AdvancedSentimentClassifier")
except Exception as e:
    print(f"❌ Import error: {e}")
    sys.exit(1)

async def test_ola_market_share_comment():
    """Test the Ola market share growth comment sentiment classification"""
    ...
    try:
        ...
        print(f"  Script Distribution: {lang_info.get('script_distribution', {})}")
        ...
        print(f"  Emojis Found: {emoji_info.get('emojis', [])}")
        ...
        print(f"  Intensity Multiplier: {pattern_info.get('intensity_multiplier', 1.0)}")
        ...
        # Engagement
        engagement_info = result.get('engagement_analysis', {})
        print(f"\n📈 Engagement Analysis:")
        print(f"  Engagement Score: {engagement_info.get('engagement_score', 0)}")
        print(f"  Engagement Level: {engagement_info.get('engagement_level', 'none')}")
        print(f"  Amplification Factor: {engagement_info.get('amplification_factor', 1.0)}")
        ...
        # Special insights for this comment
        print(f"\n💡 SPECIAL INSIGHTS:")
        print("  - Hindi-English code-switching detected")
        print("  - 'badh raha hai' = 'is increasing' (positive growth statement)")
        print("  - 💯 emoji indicates satisfaction/approval")
        print("  - Market share growth is positive business sentiment")
...
if __name__ == "__main__":
    asyncio.run(test_ola_market_share_comment())
